...
public class Plugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        damageManager = new DamageManager(this, characterManager, armorManager);
    ...
    private boolean handleCreate(CommandSender sender,
        ...
        if (args.length < 2 || !args[0].equalsIgnoreCase("session")) {
            player.sendMessage(color("&c使い方: /create session <セッション名>"));
            return true;
        }
        ...
        if (!sessionManager.createSession(sessionName, player)) {
    ...
    private boolean handleSession(CommandSender sender,
        ...
        if (args.length == 0) {
            ...
            if (keeperManager.isKeeper(player)
                ...
                player.sendMessage(color("&e/create session <名前> &7- 新しいセッションを作成"));
            }
            return true;
        }
        ...
        if (args[0].equalsIgnoreCase("join")) {
            if (!sessionManager.isActive()) {
                ...
                player.sendMessage(color("&7KPが /create session <セッション名> で作成してください。"));
                return true;
            }
        ...
        player.sendMessage(color("&c使い方: /session <join|leave|list|end>"));
        return true;
    }
